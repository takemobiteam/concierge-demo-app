...
class ConciergeTabState extends State<ConciergeTab>
  ...
  void _onBridgeMessage(JavaScriptMessage msg) {
    debugPrint('[Bridge] raw message: ${msg.message}');
    dynamic data;
    try {
      data = jsonDecode(msg.message);
    } catch (e) {
      debugPrint('[Bridge] JSON decode failed: $e');
      return;
    }
    if (data is! Map) return;

    final type = data['type'];
    debugPrint('[Bridge] received type=$type');
    ...
    switch (type) {
      ...
      case 'OPEN_PAGE':
        ...
        if (page is String) widget.onOpenPage(page);
        break;
      default:
        debugPrint('[Bridge] unhandled type: $type');
    }
  }
